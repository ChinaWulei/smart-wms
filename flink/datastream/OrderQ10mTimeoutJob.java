...
public class OrderQ10mTimeoutJob {
    ...
    public static void main(String[] args) throws Exception {
        ...
        KafkaSource<String> source = KafkaSource.<String>builder()
                ...
                .setTopics("dwd_order_status_change")
                .setGroupId("wms-dws-q-10m-timeout")
                .setStartingOffsets(OffsetsInitializer.earliest())
                .setValueOnlyDeserializer(new SimpleStringSchema())
                .build();

        DataStream<OrderStatusChange> changes = env.fromSource(source, WatermarkStrategy.noWatermarks(), "dwd-order-status-change")
    ...
    private static class QTimeoutProcessFunction extends KeyedProcessFunction<Long, OrderStatusChange, Void> {
        ...
        public void processElement(OrderStatusChange change, Context context, Collector<Void> out) throws Exception {
            ...
            if (isQ) {
                long qStart = current != null && current.qStartTime > 0 ? current.qStartTime : change.eventTimeMillis(now);
        ...
        public void onTimer(long timestamp, OnTimerContext context, Collector<Void> out) throws Exception {
            OrderRuntimeState current = state.value();
            if (current == null || current.timeoutEmitted || !isQStatus(current.currentStatus)) return;
    ...
    public static class OrderStatusChange {
        ...
        static OrderStatusChange fromJson(String json) throws Exception {
            ...
            change.orderId = node.path("order_id").isMissingNode() ? node.path("orderId").asLong() : node.path("order_id").asLong();
    ...
    private static class ClickHouseWriter implements AutoCloseable {
        ...
        ClickHouseWriter() throws Exception {
            connection = DriverManager.getConnection("jdbc:clickhouse://clickhouse:8123/smart_wms_dw", "default", "");
        ...
        private String formatTime(long epochMillis) {
            return LocalDateTime.ofInstant(Instant.ofEpochMilli(epochMillis), ZoneId.systemDefault()).toString().replace('T', ' ');
        }
